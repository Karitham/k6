...
func getCloudCmd(ctx context.Context, logger *logrus.Logger, fl *commandFlags) *cobra.Command {
...
This will execute the test on the k6 cloud service. Use "k6 login cloud" to authenticate.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if showCloudLogsEnv, ok := os.LookupEnv("K6_SHOW_CLOUD_LOGS"); ok {
				...
				if !cmd.Flags().Changed("show-logs") {
					fl.showCloudLogs = showCloudLogsValue
				}
			}
			...
			modifyAndPrintBar(progressBar, fl, pb.WithConstProgress(0, "Getting script options"))
			...
			modifyAndPrintBar(progressBar, fl, pb.WithConstProgress(0, "Consolidating options"))
			cliOpts, err := getOptions(cmd.Flags())
			if err != nil {
				return err
			}
			conf, err := getConsolidatedConfig(
				afero.NewOsFs(), Config{Options: cliOpts}, r.GetOptions(), buildEnvMap(os.Environ()), fl)
			if err != nil {
				return err
			}
			...
			modifyAndPrintBar(progressBar, fl, pb.WithConstProgress(0, "Building the archive"))
			arc := r.MakeArchive()
			// TODO: Fix this
			...
			modifyAndPrintBar(progressBar, fl, pb.WithConstProgress(0, "Validating script options"))
			...
			modifyAndPrintBar(progressBar, fl, pb.WithConstProgress(0, "Uploading archive"))
			...
			go func() {
				showProgress(progressCtx, []*pb.ProgressBar{progressBar}, logger, fl)
				progressBarWG.Done()
			}()
			...
			for range ticker.C {
				...
				if (newTestProgress.RunStatus > lib.RunStatusRunning) ||
					(fl.exitOnRunning && newTestProgress.RunStatus == lib.RunStatusRunning) {
					globalCancel()
					break
				}
			}
	...
	cloudCmd.Flags().AddFlagSet(cloudCmdFlagSet(fl))
	return cloudCmd
}

func cloudCmdFlagSet(fl *commandFlags) *pflag.FlagSet {
	...
	flags.BoolVar(&fl.exitOnRunning, "exit-on-running", fl.exitOnRunning, "exits when test reaches the running status")
	...
	flags.BoolVar(&fl.showCloudLogs, "show-logs", fl.showCloudLogs,
