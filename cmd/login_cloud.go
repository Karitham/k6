...
func getLoginCloudCommand(logger logrus.FieldLogger, fl *commandFlags) *cobra.Command {
...
This will set the default token used when just "k6 run -o cloud" is passed.`,
  ...
  k6 login cloud`[1:],
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			currentDiskConf, configPath, err := readDiskConfig(fs, fl)
			if err != nil {
				return err
			}
			...
			default:
				...
				vals, err = form.Run(os.Stdin, fl.stdout)
				if err != nil {
					return err
				}
				email := vals["Email"]
