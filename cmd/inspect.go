...
func getInspectCmd(logger *logrus.Logger, fl *commandFlags) *cobra.Command {
	...
	inspectCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if addExecReqs {
				inspectOutput, err = addExecRequirements(b, builtinMetrics, registry, logger, fl)
				if err != nil {
					return err
				}
			}
...
func addExecRequirements(b *js.Bundle,
	...
	logger *logrus.Logger, fl *commandFlags) (interface{}, error) {
	...
	conf, err := getConsolidatedConfig(afero.NewOsFs(), Config{}, runner.GetOptions(), buildEnvMap(os.Environ()), fl)
	if err != nil {
		return nil, err
	}
