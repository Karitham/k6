...
a commandline interface for interacting with it.`,
  ...
  k6 run -o influxdb=http://1.2.3.4:8086/k6`[1:],
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			conf, err := getConsolidatedConfig(afero.NewOsFs(), cliConf, initRunner.GetOptions(), buildEnvMap(os.Environ()), fl)
			if err != nil {
				return err
			}
			...
			printExecutionDescription(
				...
				executionPlan, outputs, fl.noColor || !fl.stdoutTTY, fl)
			...
			if !runtimeOptions.NoSummary.Bool {
				summaryResult, err := initRunner.HandleSummary(globalCtx, &lib.Summary{
					...
					NoColor:         fl.noColor,
					UIState: lib.UIState{
						IsStdOutTTY: fl.stdoutTTY,
						IsStdErrTTY: fl.stderrTTY,
					},
				})
				if err == nil {
					err = handleSummaryResult(afero.NewOsFs(), fl.stdout, fl.stderr, summaryResult)
				}
				if err != nil {
			...
			if conf.Linger.Bool {
				...
				default:
					...
					fprintf(fl.stdout, "Linger set; waiting for Ctrl+C...")
					<-lingerCtx.Done()
